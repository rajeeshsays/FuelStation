...
export async function saveStockEntry(
  ...
  try {
    ...
    if (id) {
      ...
      await logAction('UPDATE', `Updated stock entry for invoice ${invoiceNumber}.`);
    } else {
      ...
      await logAction('INSERT', `Added stock entry for ${liters}L of ${fuelType} (Inv: ${invoiceNumber}).`);
    }
...
export async function deleteStockEntry(prevState: DeleteStockState, formData: FormData): Promise<DeleteStockState> {
    ...
    try {
        ...
        await logAction('DELETE', `Deleted stock entry for invoice ${deleted.invoiceNumber} (${deleted.liters}L of ${deleted.fuelType}).`);
